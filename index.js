...
// Restrict command prefix to specific ascii characters https://en.cppreference.com/w/cpp/language/ascii
const validPrefix = /^[\x21-\x2F]$/;

// Set prefix & auth token to the env variable values, if defined.
// Otherwise default to the values in config.json
let guildPrefix = process.env.PREFIX || Object.values(config.prefix);
let token = process.env.TOKEN || config.token;

// Authenticate with Discord
try {
    vars.client.login(token);
    vars.client.once('ready', () => {
    console.log('[INIT] WelcomeBot Ready!');
  });
} catch (err) {
		console.error(err);
		process.exit(1);
...
vars.client.on('message', async message => {
	...
	// If the prefix is missing/invalid, default to '!' and update config.json
	if (!guildPrefix) {
		guildPrefix = '!';
		config.prefix[message.guild.id] = guildPrefix;
		try { vars.fs.writeFileSync('./config.json', JSON.stringify(config)); }
	...
	if ((command == 'p' || command == 'prefix')) {
		prefix.setGuildPrefix(args, message, guildPrefix, validPrefix, modMembers, adminMembers);
	}
	/* RESET COMMAND */ 
